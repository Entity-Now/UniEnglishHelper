...
export function getEdgeTTSTrustedClientToken(): string {
  return getRuntimeEnv("VITE_EDGE_TTS_TRUSTED_CLIENT_TOKEN") ?? EDGE_TTS_DEFAULT_TRUSTED_CLIENT_TOKEN
}

export function getEdgeTTSSignatureSecretBase64(): string {
  return (
    getRuntimeEnv("VITE_EDGE_TTS_SIGNATURE_SECRET_BASE64") ??
    EDGE_TTS_DEFAULT_SIGNATURE_SECRET_BASE64
  )
}

export const EDGE_TTS_SIGNATURE_APP_ID =
  getRuntimeEnv("VITE_EDGE_TTS_SIGNATURE_APP_ID") ?? "MSTranslatorAndroidApp"
...
export const EDGE_TTS_HTTP_ENABLED = import.meta.env.VITE_EDGE_TTS_HTTP_ENABLED !== "false"
