...
export class BatchQueue<T, R> {
  ...
  private schedule() {
    ...
    for (const [batchKey, batch] of this.pendingBatchMap.entries()) {
      ...
      if (shouldFlushNow || isTimedOut) batchesToFlush.push(batchKey);
    }
  ...
  private async executeBatchWithRetry(
    ...
    try {
      ...
      if (!results) throw new Error('Batch execution results are undefined');
      if (results.length !== tasks.length) {
        throw new BatchCountMismatchError(
          tasks.length,
          results.length,
          results,
        );
      ...
      if (retryCount < this.maxRetries && err instanceof BatchCountMismatchError) {
        const delay = Math.min(
          BASE_BACKOFF_DELAY_MS * 2 ** retryCount,
  ...
  private async executeFallbackIndividual(
    tasks: BatchTask<T, R>[],
    batchKey: string,
  ) {
  ...
  setBatchConfig(
    config: Partial<
      Pick<BatchOptions<T, R>, 'maxCharactersPerBatch' | 'maxItemsPerBatch' | 'batchDelay'>
    >,
  ) {
    this.maxCharactersPerBatch =
      config.maxCharactersPerBatch ?? this.maxCharactersPerBatch;
  ...
  getStats() {
    return {
      pendingBatches: this.pendingBatchMap.size,
      flushedCount: this.flushedCount,
      errorCount: this.errorCount,
      maxItemsPerBatch: this.maxItemsPerBatch,
      maxCharactersPerBatch: this.maxCharactersPerBatch,
    };
  }
}
